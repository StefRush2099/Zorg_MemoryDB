...
if [[ -f release/zorg-db-memory-v1.2.64.tar.gz ]]; then
  if tar -tzf release/zorg-db-memory-v1.2.64.tar.gz | rg '(^|/)(node_modules|__pycache__|\\.next|dist|tmp|browser-profile)(/|$)|sql_memory_map\\.json$|\\.(pyc|dump|backup|tar\\.gz)$'; then
    echo "generated/private artifact found inside release archive" >&2
    exit 1
  fi

  if tar -tzf release/zorg-db-memory-v1.2.64.tar.gz | rg -n 'daily-github-sync|daily GitHub sync|once per day|StefRush2099/Zorg_MemoryDB'; then
