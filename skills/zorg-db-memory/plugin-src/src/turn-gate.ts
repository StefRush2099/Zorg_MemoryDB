...
type State = {
  runId: string; sessionKey: string; prompt: string; requestHash: string;
 startedAt: number; status: "preparing"|"complete"|"recovery"|"restored";
  receiptId?: string; incidentId?: string; failure?: string;
  alert?: "down"|"restored";
  mandatoryRuleKeys?: string[];
  returnedRuleKeys?: string[];
};
...
const ruleKey = (row: pg.QueryResultRow) => {
  const content = String(row.content || row.row_data?.content || "");
  return content.match(/^Key:\s*([^\n]+)$/m)?.[1]?.trim();
};

const validateVisibleText = (text: string, startedAt: number) => {
  ...
  const errors: string[] = [];
  if (timeLines.length !== 1) errors.push(`expected exactly one Time line; found ${timeLines.length}`);
  if (!last.startsWith("Time:")) errors.push("Time line must be the absolute final line");
  if (last.includes("Pacific time")) errors.push("Time line must not contain the words Pacific time");
  if (zone && !last.includes(zone)) errors.push(`Time line must use ${zone}`);
  if (!/\((?:\d+s|\d+m \d{2}s|\d+h \d{2}m \d{2}s)\)$/.test(last)) errors.push("Time line must end with measured elapsed duration");
  if (Date.now() < startedAt) errors.push("turn start time is invalid");
  return errors;
};

const runs = new Map<string, State>();
const sessions = new Map<string, State>();
const recovery = new Map<string, State>();
const pendingRuns = new Map<string, Promise<void>>();
const pendingSessions = new Map<string, Promise<void>>();
const hash = (v: string) => createHash("sha256").update(v).digest("hex");
const ids = (ctx: {runId?:string;sessionKey?:string}, prompt: string) => ({
...
const category = (error: unknown) => {
  ...
  if (code.startsWith("08") || /connect|econnrefused|socket|network/.test(message)) return "connection_failed";
  if (/schema|relation .* does not exist|function .* does not exist/.test(message)) return "memory_schema_unavailable";
...
async function receipt(query: QueryFn, state: State, rows: pg.QueryResultRow[], recovered: boolean) {
  const id = randomUUID();
  ...
  await query(`insert into public.memory_turn_recall_receipts
    ...
    mandatory_rule_keys=excluded.mandatory_rule_keys,returned_rule_keys=excluded.returned_rule_keys`,
    [id,state.runId,state.sessionKey,state.requestHash,recovered?"recovered":"complete",
...
const normalContext = (s: State, rows: pg.QueryResultRow[]) =>
  `[ZORG MEMORYDB RECEIPT ${s.receiptId}] Authoritative PostgreSQL recall completed for this exact turn.\n${JSON.stringify(rows)}`;
const downContext = (s: State) => `[ZORG MEMORYDB RECOVERY MODE]
...
const restoredContext = (s: State, rows: pg.QueryResultRow[]) => `[ZORG MEMORYDB RESTORED]
...
const recoveryTool = (name: string) =>
  ["exec","memory","postgres","gateway","service","process","system","network","message","telegram","session_status"]
    .some(part => name.toLowerCase().includes(part));

export function registerZorgMemoryHooks(api: OpenClawPluginApi, deps: {query:QueryFn;recall:RecallFn}) {
  const prepareTurn = async (event: {prompt:string}, ctx: {runId?:string;sessionKey?:string}) => {
    const k = ids(ctx,event.prompt);
    const existing = runs.get(k.runId);
    if (existing?.requestHash === hash(event.prompt) && existing.status !== "recovery") return;
    ...
    try {
      ...
      if (!rows.length) throw new Error("memory recall returned no authoritative rows");
      if (rows.length < 8) {
      ...
      const mandatoryKeys = mandatory.map(ruleKey).filter(Boolean) as string[];
      const missing = MANDATORY_RULE_KEYS.filter(key => !mandatoryKeys.includes(key));
      if (missing.length) throw new Error(`mandatory memory rules missing: ${missing.join(",")}`);
      ...
      state.returnedRuleKeys=rows.map(ruleKey).filter(Boolean) as string[];
      const recovered = Boolean(old);
      state.status = recovered ? "restored" : "complete";
      await receipt(deps.query,state,rows,recovered);
      runs.set(k.runId,state);
      sessions.set(k.sessionKey,state);
      if (!recovered) return {prependContext:normalContext(state,rows)};
      ...
      state.status="recovery"; state.incidentId=incidentId; state.failure=category(error);
      state.alert=old ? undefined : "down";
      recovery.set(k.sessionKey,state); runs.set(k.runId,state);
      sessions.set(k.sessionKey,state);
  ...
  api.on("llm_input", async (event,ctx) => {
    ...
    if (sessionKey) pendingSessions.set(sessionKey,pending);
    try { await pending; } finally {
      if (runId) pendingRuns.delete(runId);
      if (sessionKey) pendingSessions.delete(sessionKey);
    }
  },{priority:1000});
  ...
  api.on("before_tool_call", async (event,ctx) => {
    ...
    if (!state) return {block:true,blockReason:"Zorg MemoryDB receipt is missing for this turn."};
    ...
    if (state.status==="complete") {
      if (/message|telegram/i.test(event.toolName)) {
        ...
        if (errors.length) return {block:true,blockReason:`MemoryDB response validation failed. Revise the same response and retry: ${errors.join("; ")}`};
      }
      return;
    }
    ...
    if (/message|telegram/i.test(event.toolName)) {
      if (state.alert==="down") { state.alert=undefined; return; }
  ...
  api.on("before_agent_finalize", async (event,ctx) => {
    ...
    if (!state || state.status!=="complete") return {action:"revise",reason:"MemoryDB exact-turn receipt is not complete",
    ...
    if (!errors.length) return {action:"continue"};
  ...
  api.on("reply_payload_sending", async event => {
    ...
    const state = runs.get(event.runId||"") || sessions.get((event as any).sessionKey||"");
    if (!state) return {cancel:true,reason:"Zorg MemoryDB receipt is missing."};
