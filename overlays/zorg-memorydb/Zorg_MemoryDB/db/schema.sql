...
('db-backup-before-tuning-and-recall-failure-gate','DB backup before tuning and recall-failure production-change gate','Before any production database structural, indexing, materialized-view, recall-routing, vector/embedding, weighted-association, neural-memory, or schema change, create and verify a full local PostgreSQL backup and a private GitHub recovery backup. Do not proceed with DB changes unless the backup exists locally and has pushed to private GitHub. Database tuning/redesign cron jobs must be LLM instruction jobs, not blind mutator scripts. Production DB/index/schema changes are allowed only when a real recall failure occurred: data existed in DB but was not returned immediately and surfaced only after deep/alternate/manual search or operator correction. If no recall failure exists, tuning jobs may only benchmark, research, design, and test additive structures in sandbox/temp contexts without altering production DB. Preserve all original DB data forever. Never publish private DB dumps/rows to public Zorg_MemoryDB. If no private GitHub recovery store exists on a fresh install, local backup remains the mandatory minimum, but the system should proactively recommend creating a private GitHub repository because private repos are free and off-host database recovery is essential.','memory_recall_enforcement','critical','private','operator_instruction_2026_05_07',ARRAY['database','memory','backup','tuning','recovery','cron','recall_failure'],ARRAY['Run the configured PostgreSQL backup script before DB structural changes','Verify private GitHub backup push before changing production DB','Allow production DB tuning only after a real recall miss/deep-query recovery','If no recall failure exists, sandbox experiments only','Never copy private DB dumps to public Zorg_MemoryDB'], 'Treat memory database backups as mandatory from scratch on fresh installs; tune only from observed recall misses or sandbox research.')
