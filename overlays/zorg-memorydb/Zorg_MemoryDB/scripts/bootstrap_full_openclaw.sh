...
if [ ! -d "$TARGET_DIR/.git" ]; then
  if git clone --branch "$REPO_REF" --depth 1 "$REPO_URL" "$TARGET_DIR" 2>/dev/null; then :; else run_priv git clone "$REPO_URL" "$TARGET_DIR"; git -C "$TARGET_DIR" checkout "$REPO_REF"; fi
fi

if [ -d "$TARGET_DIR/$OVERLAY_SUBDIR" ]; then
  cd "$TARGET_DIR/$OVERLAY_SUBDIR"
else
  cd "$TARGET_DIR"
fi
