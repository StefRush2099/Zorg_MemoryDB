...
Zorg MemoryDB is an additive OpenClaw overlay with DB-backed memory integrated into the normal OpenClaw home and workspace layout.

The install flow is deliberately two-stage: install the original upstream OpenClaw first, then graft Zorg MemoryDB onto that OpenClaw home/workspace before first assistant use. A fresh install should feel like starting OpenClaw from scratch: run the same public command, open OpenClaw, and get the normal startup behavior after DB-backed recall has been wired and verified. The database is an internal implementation detail, stored inside OpenClaw's own folders, and should not require separate setup or user-facing credentials.
...
4. **Standard Ubuntu** — native upstream OpenClaw first, then local integrated memory runtime.
...
When the container starts, `docker/entrypoint.sh` starts upstream OpenClaw only after the Zorg DB memory graft has completed. The sequence is:
...
11. starts `openclaw gateway run`, allowing OpenClaw to continue with its normal startup preamble after DB recall is available
