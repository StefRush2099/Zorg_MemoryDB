...
const { Pool } = pg;
type DbConfig = { postgres: pg.PoolConfig };
...
async function query(text: string, values: unknown[] = []) {
  ...
  try { return (await client.query(text, values)).rows; } finally { client.release(); }
}
...
async function ensureQueryEmbedding(queryText: string) {
  const [slot] = await query(`select embedding_provider,embedding_model,embedding_dim,endpoint
    from public.memory_embedding_model_slots where enabled and is_default order by updated_at desc limit 1`) as EmbeddingSlot[];
...
const server = new McpServer({ name: "zorg-memorydb", version: "4.1.0" });
