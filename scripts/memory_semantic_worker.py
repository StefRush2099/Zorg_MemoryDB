...
def upsert_node(cur, kind: str, label: str, basis: str, confidence: float):
    ...
    cur.execute(
        ...
        on conflict (node_key) do update set
          aliases = (select array(select distinct unnest(public.memory_semantic_nodes.aliases || excluded.aliases))),
          description = coalesce(public.memory_semantic_nodes.description, excluded.description),
          llm_hint = coalesce(public.memory_semantic_nodes.llm_hint, excluded.llm_hint),
          confidence = greatest(coalesce(public.memory_semantic_nodes.confidence,0), coalesce(excluded.confidence,0)),
...
def insert_edge(cur, subject_type: str, subject_key: str, relation: str, object_type: str, object_key: str, weight: float, basis: str, reason: str, evidence_source: str):
    ...
    if existing:
        cur.execute(
            ...
            set weight=greatest(weight,%s), weight_basis=%s, llm_reason=%s, evidence_source=%s, updated_at=now()
            where id=%s
            """,
            (weight, basis, reason, evidence_source, existing["id"]),
        )
    else:
        cur.execute(
            """
