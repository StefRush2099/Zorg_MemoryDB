...
const healthyQuery = vi.fn(async (sql) => sql.includes("from public.zorg_logic_rules") ? mandatoryRows : []);
describe("Zorg MemoryDB turn gate", () => {
    it("writes a receipt and injects authoritative recall", async () => {
        ...
        registerZorgMemoryHooks(a.api, { query: query, recall });
        const out = await a.hooks.get("agent_turn_prepare")({ prompt: "hello", messages: [], queuedInjections: [] }, { runId: "r1", sessionKey: "s1" });
    ...
    it("prepares Codex llm_input turns and allows the matching tool call", async () => {
        ...
        const recall = vi.fn(async () => Array.from({ length: 8 }, (_, i) => ({ row_data: { i } })));
        registerZorgMemoryHooks(a.api, { query: query, recall });
        await a.hooks.get("llm_input")({ prompt: "codex", runId: "r-codex" }, { runId: "r-codex", sessionKey: "s-codex" });
        const gate = a.hooks.get("before_tool_call");
        expect(await gate({ toolName: "memory_search", params: {}, runId: "r-codex" }, { runId: "r-codex", sessionKey: "s-codex" })).toBeUndefined();
    });
    it("waits for an in-flight Codex receipt before allowing tools", async () => {
        const a = fakeApi();
        let release;
        const recall = vi.fn(() => new Promise(resolve => {
            release = () => resolve(Array.from({ length: 8 }, (_, i) => ({ row_data: { i } })));
        }));
        registerZorgMemoryHooks(a.api, { query: healthyQuery, recall });
        const preparing = a.hooks.get("llm_input")({ prompt: "slow", runId: "r-slow" }, { runId: "r-slow", sessionKey: "s-slow" });
        const gated = a.hooks.get("before_tool_call")({ toolName: "memory_search", params: {}, runId: "r-slow" }, { runId: "r-slow", sessionKey: "s-slow" });
    ...
    it("blocks a recovery tool when no receipt state exists", async () => {
        const a = fakeApi();
        registerZorgMemoryHooks(a.api, { query: vi.fn(), recall: vi.fn() });
        const gate = a.hooks.get("before_tool_call");
        expect((await gate({ toolName: "memory_search", params: {}, runId: "missing" }, { runId: "missing", sessionKey: "missing" })).block).toBe(true);
    });
    it("allows one down alert and blocks ordinary work", async () => {
        ...
        registerZorgMemoryHooks(a.api, { query: failed, recall: vi.fn() });
    ...
    it("sends an invalid draft back to the same model for revision", async () => {
        const a = fakeApi();
        registerZorgMemoryHooks(a.api, { query: healthyQuery, recall: vi.fn(async () => Array.from({ length: 8 }, (_, i) => ({ row_data: { i } }))) });
