...
if ! have sudo && [ "$(id -u)" -ne 0 ]; then
  echo "sudo is required for the native Linux install." >&2
  exit 1
fi
...
ensure_node(){
  ...
  if ! have npm; then
    echo "npm is missing after Node.js install." >&2
    exit 1
  fi
}
...
if [ ! -d "$INSTALL_DIR/.git" ]; then
  ...
  if git clone --branch "$REPO_REF" --depth 1 "$REPO_URL" "$INSTALL_DIR" 2>/dev/null; then
    ...
    run_priv git clone "$REPO_URL" "$INSTALL_DIR"
    git -C "$INSTALL_DIR" checkout "$REPO_REF"
    run_priv chown -R "${USER:-$(id -un)}:${USER:-$(id -un)}" "$INSTALL_DIR" 2>/dev/null || true
  fi
fi
...
echo "Native Linux OpenClaw + Zorg MemoryDB install complete."
...
echo "  cd $ZORG_RUNTIME_DIR && source .env.native && OPENCLAW_WORKSPACE=$ZORG_RUNTIME_DIR SQL_MEMORY_MAP=$ZORG_RUNTIME_DIR/sql_memory_map.json openclaw gateway run --allow-unconfigured --bind \$OPENCLAW_GATEWAY_BIND --port \$OPENCLAW_GATEWAY_PORT --auth \$OPENCLAW_GATEWAY_AUTH"
