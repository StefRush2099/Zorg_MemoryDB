...
if [[ -f release/zorg-db-memory-v1.2.69.tar.gz ]]; then
  if tar -tzf release/zorg-db-memory-v1.2.69.tar.gz | rg '(^|/)(node_modules|__pycache__|\\.next|dist|tmp|browser-profile)(/|$)|sql_memory_map\\.json$|\\.(pyc|dump|backup|tar\\.gz)$'; then
    echo "generated/private artifact found inside release archive" >&2
    exit 1
  fi

  if tar -tzf release/zorg-db-memory-v1.2.69.tar.gz | rg -n 'daily-github-sync'; then
    echo "operator-only daily GitHub sync wording found inside public archive" >&2
    exit 1
  fi

  if tar -tzf release/zorg-db-memory-v1.2.69.tar.gz \
    | rg '\.(md|txt|json|sh|py|sql|cjs|mjs|ts|tsx|js|css|html|ya?ml)$' \
    | rg -v '^scripts/verify-public-package\.sh$' \
    | while IFS= read -r archive_path; do tar -xOzf release/zorg-db-memory-v1.2.69.tar.gz "$archive_path"; done \
