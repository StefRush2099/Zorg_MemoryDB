...
def classify(e):
    ...
    if t in ('assistant.message','assistant_message') or e.get('role')=='assistant': return 'model_output', {'content':safe(e.get('content',d)),'metadata':{'source_type':t,'seq':e.get('seq')}}
        ...
        for path in paths:
            ...
            with open(path,encoding='utf-8',errors='replace') as fh:
                for line in fh:
                    ...
                    seen+=1; digest=hashlib.sha256(line.encode()).hexdigest()
                    kind,payload=classify(e); turn=str(e.get('traceId') or e.get('sessionId') or path.stem)
